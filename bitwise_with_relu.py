...
output_relu = helper.make_tensor_value_info("output_relu", TensorProto.INT32, [None])  # <- New output
...
node_relu = helper.make_node("Relu", ["output_xor"], ["output_relu"])  # <- Apply ReLU to XOR output

# Create graph
graph_def = helper.make_graph(
    nodes=[node_and, node_or, node_xor, node_relu],
    name="BitwiseLogicWithReLU",
    inputs=[input1, input2],
    outputs=[output_and, output_or, output_xor, output_relu],
)

# Create model
model_def = helper.make_model(graph_def, producer_name="bitwise-relu")

# Save model
onnx.save(model_def, "bitwise_with_relu.onnx")